...
public class UrlService
{
    ...
    {
        return _urls.Find(Url => Url.ID == id).FirstOrDefault();
    }

    public List<Url> GetUserUrls(User user)
    {
        return Get().Where(u => u.Owner == user.Username).ToList();
    }
    ...
    public void Update(string id, Url UrlIn)
    {
        _urls.ReplaceOne(Url => Url.ID == id, UrlIn);
    }

    public void Remove(Url UrlIn)
    {
        _urls.DeleteOne(Url => Url.ID == UrlIn.ID);
    }

    public void Remove(string id)
    {
        _urls.DeleteOne(Url => Url.ID == id);
    }
}
