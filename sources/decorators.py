	...
	def decorator(f):
		...
		async def decorated_function(request, *args, **kwargs):
			...
			if (userData["is_banned"] == True): 
				logger.warning(f"{userData['username']} e bannato e sta tentando di accedere ({request.headers.get("HTTP_X_FORWARDED_FOR")})")
			...
			if user == 'anonymous':
				...
				if not request.headers.get("HTTP_X_FORWARDED_FOR") in jsonValue['anonymous']: jsonValue['anonymous'][request.headers.get("HTTP_X_FORWARDED_FOR")] = 0
				if jsonValue['anonymous'][request.headers.get("HTTP_X_FORWARDED_FOR")] <= 20:
					jsonValue['anonymous'][request.headers.get("HTTP_X_FORWARDED_FOR")] += 1
					...
					jsonModule.dump(jsonValue, jsonw, indent = 2, sort_keys = True)
					jsonw.close()
					if (jsonValue['anonymous'][request.headers.get("HTTP_X_FORWARDED_FOR")] >= 50):
						if (jsonValue['anonymous'][request.headers.get("HTTP_X_FORWARDED_FOR")] == 50):
							app.ctx.webhook.post(content = f"⚠️ | Warning, {request.headers.get("HTTP_X_FORWARDED_FOR")} done 50 request in less than a minute") 
						if (jsonValue['anonymous'][request.headers.get("HTTP_X_FORWARDED_FOR")] == 100):
							app.ctx.webhook.post(content = f"ℹ️ | Just for information, I can't ban {request.headers.get("HTTP_X_FORWARDED_FOR")} but he / she did 100 requests in less than a minute")
							logger.warning(f"{request.headers.get("HTTP_X_FORWARDED_FOR")} e bannato e sta tentando di accedere")
				...
				if jsonValue[user] <= 20:
					...
					if (jsonValue[user] >= 50):
						...
						if (jsonValue[user] == 100):
							...
							logger.warning(f"{user} e bannato e sta tentando di accedere ({request.headers.get("HTTP_X_FORWARDED_FOR")})")
	...
	def decorator(f):
		...
		async def decorated_function(request, *args, **kwargs):
			...
			if userData["is_banned"]:
				logger.warning(f"{user} e bannato e sta tentando di accedere ({request.headers.get("HTTP_X_FORWARDED_FOR")})")
				return json({ "banned": "You are banned from api, you can try to contact fasmga staff to get unban" }, 401)
			if not request.headers.get("HTTP_X_FORWARDED_FOR") == "127.0.0.1":
				await app.ctx.db.users.find_one_and_update({ "api_token": request.form.get("token") }, { "$set": { "is_banned": True }})
				...
				logger.warning(f"{userData['username']} e stato temp bannato ({request.headers.get("HTTP_X_FORWARDED_FOR")})")
